...
public class adminLogin extends JFrame {
	...
	public adminLogin() {
		...
		panel.setBounds(10, 28, 862, 156);
		contentPane.add(panel);
		panel.setLayout(null);
		
		JLabel label = new JLabel("");
		label.setIcon(new ImageIcon(adminLogin.class.getResource("/photo/adminlogo.png")));
		label.setBounds(25, 11, 406, 110);
		panel.add(label);
		
		JLabel label_1 = new JLabel("");
		label_1.setIcon(new ImageIcon(adminLogin.class.getResource("/photo/download (1).jpg")));
		label_1.setBounds(598, 0, 254, 156);
		panel.add(label_1);
				...
				try {
					...
					Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/miniproject2","root","");
					
								{
											...
											String sql="SELECT * FROM `admin` WHERE `name`='"+textField.getText()+"'and " + "password='"+passwordField.getText()+"';";
											...
											if(rs.next()) {
															...
															dispose();
															AdminMainPage g=new AdminMainPage();
															a=textField.getText();
															g.main(null);
		...
		button.setBounds(366, 401, 105, 40);
		contentPane.add(button);
		
		JMenuBar menuBar = new JMenuBar();
		menuBar.setBounds(20, -8, 117, 40);
		...
		JMenuItem mntmBack = new JMenuItem("Home");
